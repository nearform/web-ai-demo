...
// Cap on one reply, for the same reason as the wllama spike: an uncapped turn
// runs to EOG or to the end of the context window.
...
// The model policy from ../../MODELS.md, applied to a catalog we read at runtime
// rather than to a hardcoded list. Each family maps to the prefix of the newest
// generation we accept; anything else in that family is superseded and dropped.
// Gemma is absent on purpose — see isGemma4Plus.
...
runSpike({
  ...
  generate: async ({
    ...
    // completes normally rather than throwing — which is why the harness reads
    ...
    const onAbort = () => {
      log.info("interruptGenerate() — undocumented, watching whether it lands");
    ...
    const chunks = await handle.chat.completions.create({
      ...
      // Same reason as the wllama spike: uncapped, one reply can consume the
      // context budget that the multi-turn test is trying to measure.
